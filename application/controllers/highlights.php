...
class Highlights extends CI_Controller {
	...
	{
		$info->catid = $this->m_content_category->load('vietnam-highlights')->id;
		...
		$tmpl_content = "";
		$tmpl_content['content']   = $this->load->view("highlight/index", $view_data, TRUE);
		$this->load->view('layout/news', $tmpl_content);
	}
	
	public function vietnam($alias=NULL)
	{
		$info->alias = $alias;
		$item = $this->m_content->getItem($info, 1);
		
		$review_info->category_id	= 23; // Vietnam highlight
		...
		$tmpl_content['content']   = $this->load->view("highlight/detail", $view_data, TRUE);
		$this->load->view('layout/news', $tmpl_content);
	}
}
