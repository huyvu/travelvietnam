...
<div id="element-box">
	...
	<div class="m">
		<form method="POST" action="<?=site_url("administrator/update_tours")?>" name="adminForm">
			<div id="editcell">
							...
							foreach ($items as $item) {
								?>
									<tr>
										...
										<td class="title">
											<a href="<?=site_url("administrator/edit_tour/".$item->id)?>"><?=$item->name?></a>
										</td>
										<td>
											<?=(($item->throughout)?"Throughout":"Daily")?>
										</td>
										<td>
											<?=$this->m_tour_category->load($item->category_id)->name?>
										</td>
										<td align="center">
											<?=$this->m_tour_destination->load($item->depart_from)->name?>
										</td>
										<td align="center">
											<?=$this->m_tour_destination->load($item->going_to)->name?>
										</td>
										<td align="center">
											<a title="add rate" href="<?=site_url("administrator/tour_itineraries/".$item->id)?>"><?=$item->duration?> d</a>
										</td>
										<td align="center">
											$<?=$item->price?>
										</td>
										<td align="center">
											...
											<a title="add rate" href="<?=site_url("administrator/tour_rates/".$item->id)?>"><?=sizeof($this->m_tour_rate->getItems($info))?></a>
										</td>
										<td align="center">
											<? $info->category_id = 1; $info->ref_id = $item->id; ?>
											<a title="upload photo" href="<?=site_url("administrator/photos/{$info->category_id}/".$item->id)?>"><?=sizeof($this->m_photo->getItems($info))?></a>
										</td>
										<td align="center">
											<? $info->tour_id = $item->id; ?>
											<a title="add trip note" href="<?=site_url("administrator/tour_tripnotes/".$item->id)?>"><?=sizeof($this->m_tour_tripnote->getItems($info))?></a>
										</td>
										<td align="center">
											<? $info->tour_id = $item->id; ?>
											<a title="add departure date" href="<?=site_url("administrator/tour_departures/".$item->id)?>"><?=sizeof($this->m_tour_departure->getItems($info))?></a>
										</td>
