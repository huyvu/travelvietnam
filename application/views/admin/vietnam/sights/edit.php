<?
	...
	$city 			= isset($item->city) ? explode(';', $item->city) : array();
	...
	// $destinations	= $this->m_vietnam_destination->getItems();
?>
...
<div id="toolbar-box">
	...
	<div class="m">
		...
		<div class="header icon-48-generic">
			Sight: <small><small>[ Edit ]</small></small>
		</div>
		<div class="clr"></div>
	</div>
	<div class="b">
...
<div id="element-box">
	...
	<div class="m">
		...
		<form id="adminForm" name="adminForm" method="post" action="<?=site_url("administrator/update_sights")?>">
										...
										<td width="100%">
											...
											<? for ($i=0; $i<20; $i++) { ?>
												<li style="width: 100px; float: left">
													<select id="city_<?=$i?>" name="city_<?=$i?>">
														<option value="">---</option>
														<? foreach ($destinations as $d) {
															echo '<option value="'.$d->id.'">'.$d->name.'</option>';
														}?>
													</select>
													<script> setValueHTML("city_<?=$i?>", '<?=$city[$i]?>'); </script>
												</li>
											<? } ?>
											</ul>
										</td>
