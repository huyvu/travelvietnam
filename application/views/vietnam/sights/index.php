<?
	if (isset($dest)) {
		...
		$entertainments = $this->m_entertainment->getItems(1);
...
<div class="inner content-13x20">
	...
	<h1 class="page-title"><?=(isset($dest))?strtoupper($dest->title)."'S ATTRACTIONS":"SIGHTS IN VIETNAM"?></h1>
			...
			<div class="sights">
				...
				<div style="overflow: inherit;position: relative;float: left;">
					...
					<div class="destination destination<?=$idx?> row<?=$row?> <?=(($row>1)?"none":"")?>">
						...
						<div class="thumbnail">
							<a title="<?=$item->title?>" href="<?=site_url("vietnam/sights/{$item->alias}")?>">
						...
						<div class="content">
							...
							<h1 class="title">
								<a title="<?=$item->title?>" href="<?=site_url("vietnam/sights/{$item->alias}")?>"><?=$item->title?></a>
							</h1>
							<p class="reviews">
	...
	<div class="right">
		<div class="destination-nav">
			<ul>
				<?if (isset($dest)) {?>
					<li><a href="<?=(sizeof($items)!=0)?site_url("vietnam/sights/{$dest->alias}"):site_url("vietnam/sights")?>" class="attraction">Attractions <span>(<?=sizeof($items)?>)</span></a></li>
		...
		<div id="todo-filter-widget">
			<div class="container">
				<form id="filter-form" action="<?=site_url("vietnam/sights")?>" method="POST">
