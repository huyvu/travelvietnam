...
function file_post_get_html($postUrl, $fields, $getUrl, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT)
{
	$curl = curl_init();
	...
	curl_setopt($curl, CURLOPT_CONNECTTIMEOUT, 30); // Time to wait for connection
	curl_setopt($curl, CURLOPT_TIMEOUT, 120); // Time to wait for whole operation
	curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1); // Return transfer instead of printing
	curl_setopt($curl, CURLOPT_FAILONERROR, 1);
	curl_setopt($curl, CURLOPT_POST, 1);
	curl_setopt($curl, CURLOPT_POSTFIELDS, $fields);
	curl_setopt($curl, CURLOPT_COOKIEJAR, 'travelovietnam.txt');
	curl_setopt($curl, CURLOPT_COOKIEFILE, 'travelovietnam.txt');
	curl_setopt($curl, CURLOPT_COOKIESESSION, 1);
	
	curl_setopt($curl, CURLOPT_SSLVERSION, 3);
	curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, 0);
	curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 2);
	curl_setopt($curl, CURLOPT_CAINFO, getcwd() . "/crt/thawtePrimaryRootCA.crt");
...
//	if( ( $contents = curl_exec($curl) ) === false )   {         //failure
//		return null;
//	}
//	
//	curl_close($curl);
...
//    $dom = new simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, DEFAULT_SPAN_TEXT);
//    
