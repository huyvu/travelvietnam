...
$(document).ready(function()
{
	...
	$(".confirm-btn").click(function(){
		...
		for (var i=1; i<=travelers; i++) {
			...
			if ($("#startdate_"+i).val() == "") {
				$("#startdate_"+i).parent().addClass("error");
				err++;
			} else {
				$("#startdate_"+i).parent().removeClass("error");
			}
		}
...
function createEntries()
{
	...
	for (var i=1; i<=travelers; i++) {
		...
		$("#startdate_"+i).datepicker(startdateoptions);
		
		$("#birthdate_"+i).change(function(){
			createEntries();
		});
...
function calFees()
{
    ...
    for (var i=1; i<=travelers; i++) {
    	...
    	p['classtype_'+i] = $('#classtype_'+i).val();
	}
	
    $('.totalfee_txt').html("...");
	$('.totalfee_txt').load("<?=site_url('tours/cal_fees')?>", p, function(fee){
...
<div class="inner">
	<div id="booking">
		<form action="<?=site_url("tours/checkout")?>" method="POST">
			...
			<div class="traveler-information">
				...
				<div class="left">
					<div class="tour-information">
						...
						<div class="duration"><b>Duration:</b> <?=($item->duration > 1) ? $item->duration." days - ".($item->duration-1)." nights" : $item->duration." day"?></div>
						<div class="destination"><b>Destinations:</b>
							<?
						...
						<table id="tbtraveler" width="100%" cellpadding="0" cellspacing="0">
							...
							<tbody class="selected">
								<tr id="traveler_1">
									...
									<td>
										<span class="calendar"><input type="text" id="startdate_1" name="startdate_1" placeholder="mm/dd/yyyy"></span>
									</td>
									<td>
										<select id="classtype_1" name="classtype_1" style="width: 150px">
											<?
												$ratenames = array();
			...
			<input type="hidden" name="tour_id" id="tour_id" value="<?=$item->id?>" />
		</form>
	</div>
	<div class="clr"></div>
</div>
