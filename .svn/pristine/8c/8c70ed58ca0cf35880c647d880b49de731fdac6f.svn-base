...
<?
	...
	$restaurant_category = $this->m_restaurant_category->load($item->catid);
?>
<div class="inner content-13x20">
	<div id="breadcrumbs">
		...
		<img alt="" src="<?=IMG_URL?>arrow.png">
		<a class="pathway" title="Vietnam" href="<?=site_url("vietnam/restaurants/{$dest->alias}")?>">Restaurants of <?=$destination->name?></a>
		...
		<div class="sight">
				...
				<div style="padding: 10px; background: #FFF; overflow: hidden">
					...
					<div class="left highlights">
						<ul>
							...
							<li><span>Type :</span> <?=$restaurant_category->name?></li>
			...
			<div class="right">
				<div id="popular-attractions">
					<div class="title">
						<h2>Popular <?=$destination->name?> Restaurants</h2>
					</div>
					<ul>
						<?
						$idx = 0;
						foreach($restaurants as $restaurant) {
							$idx++
						?>
							<li class="<?=($idx>7)?'extra-attraction none':''?>"><a href="<?=site_url("vietnam/restaurants/{$restaurant->alias}")?>"><?=$restaurant->title?></a></li>
						<? } ?>
					</ul>
					<?if(sizeof($restaurants) > 7) :?>
						<span>
