...
<div id="element-box">
	...
	<div class="m">
		<form action="<?=site_url("administrator/update_promotion")?>" method="post" name="adminForm">
			<div id="editcell">
						...
						if (!empty($items) && sizeof($items)) {
							...
							foreach ($items as $item) {
								...
								<tr class="row1">
									...
									<td align="center">
										<input type="checkbox" id="cb<?=$idx?>" name="cid[]" value="<?=$item->code?>" onclick="isChecked(this.checked);" />    			
									</td>
									<td class="title" width="60%">
										<a href="<?=site_url("administrator/edit_promotion/".$item->code)?>"><?=$item->code?></a>
									</td>
									<td align="center">
										<?=$item->visa_type?>
									</td>
