...
<div id="element-box">
	...
	<div class="m">
		<form method="POST" action="<?=site_url("administrator/update_hotels")?>" name="adminForm">
			<div id="editcell">
							...
							foreach ($items as $item) {
								?>
									<tr>
										...
										<td class="title">
											<a href="<?=site_url("administrator/edit_hotel/".$item->id)?>"><?=$item->name?></a>
										</td>
										<td align="center">
											<?=($item->accommodation_type == 1) ? "Hotel" : (($item->accommodation_type == 2) ? "Resort" : "")?>
										</td>
										<td>
											<?=$this->m_hotel_category->load($item->category_id)->name?>
										</td>
										<td align="center">
											<?=$item->star?>
										</td>
										<td align="center">
											<?=$this->m_hotel_destination->load($item->city)->name?>
										</td>
										<td align="center">
											$<?=$item->price?>
										</td>
										<td align="center">
											<? $info->hotel_id = $item->id; ?>
											<a title="add room" href="<?=site_url("administrator/rooms/".$item->id)?>"><?=sizeof($this->m_room->getItems($info))?></a>
										</td>
										<td align="center">
											<? $info->category_id = 3; $info->ref_id = $item->id; ?>
