...
<div id="element-box">
	...
	<div class="m">
		<form method="POST" action="<?=site_url("administrator/update_tour_rates_advanced")?>" name="adminForm">
					...
					<tr>
						<?
							...
							foreach ($arrtype as $type => $valtype) {
								...
								echo '<td class="first"><a href="'.site_url("administrator/edit_tour_rates/".$tour->id.'/'.$valtype).'">'.$valtype.'</a></td>';
								foreach ($arrduration as $duration => $valduration) {
									$hasrate = 0;
									foreach ($items as $rate) {
										if ($rate->name == $valtype && $rate->group_size == $valduration && !$rate->single_supplement) {
											echo '<td><a href="'.site_url("administrator/edit_tour_rate/".$rate->id).'">$'.number_format($rate->price,0,',','.').'</a></td>';
											$hasrate = 1;
											break;
										}
									}
