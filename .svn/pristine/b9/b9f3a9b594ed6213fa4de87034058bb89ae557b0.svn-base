...
<div id="element-box">
	...
	<div class="m">
		<form method="POST" action="<?=site_url("administrator/update_album_photos")?>" name="adminForm">
							...
							foreach ($items as $item) {
								?>
									<tr>
										...
										<td class="title">
											<a href="<?=site_url("administrator/edit_album_photo/".$item->id)?>"><?=$item->name?></a>
										</td>
										<td>
											<a href="<?=site_url("administrator/albums/".$item->album_id)?>"><?=$this->m_album->load($item->album_id)->name?></a>
										</td>
