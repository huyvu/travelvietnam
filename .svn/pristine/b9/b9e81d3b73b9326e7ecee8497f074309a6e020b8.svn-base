...
<?
	...
	$shopping_category = $this->m_shopping_category->load($item->catid);
?>
<div class="inner content-13x20">
	<div id="breadcrumbs">
		...
		<img alt="" src="<?=IMG_URL?>arrow.png">
		<a class="pathway" title="Vietnam" href="<?=site_url("vietnam/shopping/{$dest->alias}")?>">Shopping of <?=$destination->name?></a>
		...
		<div class="sight">
				...
				<div style="padding: 10px; background: #FFF; overflow: hidden">
					...
					<div class="left highlights">
						<ul>
							...
							<li><span>Type :</span> <?=$shopping_category->name?></li>
			...
			<div class="right">
				<div id="popular-attractions">
					<div class="title">
						<h2>Popular <?=$destination->name?> Shopping Center</h2>
					</div>
					<ul>
						<?
						$idx = 0;
						foreach($shoppings as $shopping) {
							$idx++
						?>
							<li class="<?=($idx>7)?'extra-attraction none':''?>"><a href="<?=site_url("vietnam/shopping/{$shopping->alias}")?>"><?=$shopping->title?></a></li>
						<? } ?>
					</ul>
					<?if(sizeof($shoppings) > 7) :?>
						<span>
